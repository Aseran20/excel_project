...
import type { AlgoSheetRequest, AlgoSheetResponse, QueueStats, QueuedRequest } from "@algosheet/shared";
...
class QueueManager {
  ...
  private maxConcurrency = 5; // Default: 5 concurrent requests
  private inFlightCount = 0;

  // Rate limiting: 50 requests per minute
  private rateLimitWindow = 60000; // 1 minute in ms
  private maxRequestsPerWindow = 50;
  ...
  private async processRequest(request: QueuedRequest): Promise<any> {
    ...
    console.log(
      `[QueueManager] Processing request ${request.id} (${this.inFlightCount}/${this.maxConcurrency} in flight)`
    );
    ...
    try {
      ...
      console.log(
        `[QueueManager] Request ${request.id} completed in ${request.duration}ms ${request.cacheHit ? "(cached)" : ""}`
      );
  ...
  /**
   * Make tRPC request with timeout
   */
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(
          () => reject(new Error(`Request timeout after ${this.TIMEOUT_MS / 1000} seconds - AI taking too long`)),
          this.TIMEOUT_MS
        );
      });

      // Race between tRPC call and timeout
      const result = await Promise.race([
        trpcClient.algosheet.query.mutate({
          prompt: request.prompt,
          responseMode: request.responseMode as "free" | "structured" | null | undefined,
      ...
      response._duration = Date.now() - (request.startTime || Date.now());
      response._cached = response._duration < 500;
      ...
      // Handle tRPC errors
      if (error.data?.code) {
        throw new Error(`[${error.data.code}] ${error.message}`);
      }
  ...
  private async waitForRateLimit(): Promise<void> {
    ...
    while (this.requestTimestamps.length >= this.maxRequestsPerWindow) {
      ...
      console.log(
        `[QueueManager] Rate limit reached (${this.maxRequestsPerWindow}/${this.rateLimitWindow}ms), waiting ${waitTime}ms`
      );
      ...
      this.requestTimestamps = this.requestTimestamps.filter((ts) => newNow - ts < this.rateLimitWindow);
    }

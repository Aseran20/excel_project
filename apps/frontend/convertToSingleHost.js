...
async function updatePackageJsonForSingleHost(host) {
  ...
  Object.keys(content.scripts).forEach((key) => {
  ...
  Object.keys(content.devDependencies).forEach((key) => {
...
async function updateLaunchJsonFile(host) {
  ...
  const content = JSON.parse(launchJsonContent);
  content.configurations = content.configurations.filter((config) => config.name.startsWith(getHostName(host)));

...
async function testAlgoSheet(prompt: string, mode = "free", schema = "", options = "") {
  console.log("\n" + "=".repeat(80));
...
async function runTests() {
  ...
  await testAlgoSheet("Revenue of Microsoft 2023 in billions", "structured", "number", "web=true");

  // Test 2: Recent event with web search
  await testAlgoSheet("Who won Euro 2024?", "free", "", "web=true");

  // Test 3: Number without web search (for comparison)
  await testAlgoSheet("What is 15 + 27?", "structured", "number", "");

  // Test 4: Financial metric with web search
  await testAlgoSheet("P/E ratio of Tesla", "structured", "number", "web=true");
